...
class URLTranslater(object):
	...
	def getPPtvURL(self, url):
		...
		if url.find("http://web-play.pptv.com/web-m3u8") != -1:
			return url
...
class httpLive(object):
	...
	def probe(self, url):
		...
		if url.find(".m3u8") != -1 or url.find("=m3u8") != -1 or url.find("-m3u8") != -1 or url.find("/m3u8") != -1:
...
class hlsLive(object):
	...
	def getM3u8(self, url):
		...
		try:
			...
			ts_list = []
			while count < len(arr) and len(ts_list) < 7:
