...
class EasyUrl:
	def __init__(self, url, headers={}, post_data=None):
		...
		if headers:
			...
			self.headers = {"User-Agent": "Mozilla/5.1 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.154 Safari/537.36"}
	...
	def get_response(self):
		self.req = urllib2.Request(url=self.url, data=self.post_data, headers=self.headers)
	...
	def check_url(self, check_times=3, timeout=5):
	    for i in xrange(check_times):
	        print "**", i+1, "**", self.url
	        try:
	            self.checked_result = urllib2.urlopen(url=self.url, timeout=timeout)
